...
class XUIManager:
    ...
    def login(self):
        ...
        if self.original_url.startswith('http://'): protocols.append(self.original_url.replace('http://', 'https://'))
...
def generate_node_link(node, server_host):
    try:
        ...
        if net == 'ws': path = st.get('wsSettings',{}).get('path','/'); host = st.get('wsSettings',{}).get('headers',{}).get('Host','')
...
def sub_handler(token: str):
    ...
    for srv in SERVERS_CACHE:
        try:
            mgr = get_manager(srv); inbounds = mgr.get_inbounds()
...
async def open_add_server_dialog():
    with ui.dialog() as d, ui.card().classes('w-96'):
        ...
        n=ui.input('名称').classes('w-full'); g=ui.input('分组 (选填)').classes('w-full'); u=ui.input('URL').classes('w-full')
        us=ui.input('账号').classes('w-full'); p=ui.input('密码', password=True).classes('w-full')
        ...
        async def save():
            ...
            # 4. 【修复】强制跳转到新服务器的详情页，让用户能立刻看到效果
            refresh_content('SINGLE', new_server)
...
async def open_edit_server_dialog(idx):
    ...
    with ui.dialog() as d, ui.card().classes('w-96'):
        ...
        n=ui.input('名称', value=data['name']).classes('w-full'); g=ui.input('分组', value=data.get('group','')).classes('w-full')
        ...
        async def save():
            # 更新缓存
            updated_server = {'name':n.value,'group':g.value,'url':u.value,'user':us.value,'pass':p.value,'prefix':pre.value}
            SERVERS_CACHE[idx] = updated_server
            await save_servers()
            refresh_sidebar() 
            # 刷新当前页面显示新数据
            refresh_content('SINGLE', updated_server)
            d.close()
    ...
    with ui.dialog() as d, ui.card().classes('w-[500px] h-[70vh] flex flex-col p-0 gap-0'):
        ...
        with ui.column().classes('w-full flex-grow overflow-y-auto p-0 gap-0'):
            for i, s in enumerate(SERVERS_CACHE):
                ...
                with ui.row().classes(f'w-full items-center px-4 py-2 {bg_color} hover:bg-blue-50 border-b border-gray-100'):
        ...
        with ui.row().classes('w-full p-4 border-t bg-gray-50 justify-end'):
            async def save():
                ...
                await save_servers()
                refresh_sidebar(); refresh_content('GROUP', new_name.value); d.close()
...
async def open_create_group_dialog():
    with ui.dialog() as d, ui.card().classes('w-80'):
        ...
        async def confirm():
            if name_input.value:
                d.close(); await open_group_mgmt_dialog(name_input.value)
            else: ui.notify('请输入分组名称', type='warning')
    ...
    with sidebar_container:
        ...
        with ui.column().classes('w-full flex-grow overflow-y-auto p-2 gap-1'):
            ...
            for gname, gservers in groups.items():
                with ui.expansion('', icon='folder').classes('w-full border rounded mb-1 bg-white shadow-sm').props('expand-icon-class=hidden') as exp:
                    ...
                    with ui.column().classes('w-full gap-0'):
                        for srv in gservers:
                            with ui.row().classes('w-full justify-between p-2 pl-4 cursor-pointer hover:bg-blue-50 items-center border-t border-gray-100 no-wrap'):
                                ...
                                ui.button(icon='edit', on_click=lambda i=idx: open_edit_server_dialog(i)).props('flat dense round size=xs color=grey')

        with ui.column().classes('w-full p-2'): pass
...
def refresh_content(scope='ALL', data=None):
    ...
    title = ""
    if scope == 'ALL': 
        targets = SERVERS_CACHE; title = f"🌍 所有节点 ({len(targets)})"
    elif scope == 'GROUP':
        targets = [s for s in SERVERS_CACHE if s.get('group', '默认分组') == data]; title = f"📁 分组: {data}"
    elif scope == 'SINGLE':
        targets = [data]; title = f"🖥️ {data['name']}"
    ...
    with content_container:
        ...
        if scope == 'SINGLE': render_single_server_view(data)
        else: render_aggregated_view(targets)

def render_single_server_view(server_conf):
    mgr = get_manager(server_conf)
    with ui.row().classes('w-full mb-4 justify-end'):
        ui.button('新建节点', icon='add', color='green', on_click=lambda: open_inbound_dialog(mgr, None, lambda: refresh_content('SINGLE', server_conf)))
    
    list_container = ui.column().classes('w-full')
    async def load_nodes():
        res = await fetch_inbounds_safe(server_conf); list_container.clear()
        ...
        if '://' not in raw_url: raw_url = f'http://{raw_url}'
        try: base = f"{urlparse(raw_url).scheme}://{urlparse(raw_url).hostname}:{urlparse(raw_url).port or 80}"
        except: base = None
        ...
        with list_container:
            ...
            with ui.element('div').classes('grid w-full gap-4 font-bold text-gray-500 border-b pb-2 px-2').style(cols):
                for h in ['名称', '所在组', '协议', '端口', '状态', '延迟', '操作']: ui.label(h).classes('w-full text-center')
            ...
            for n in res:
                with ui.element('div').classes('grid w-full gap-4 py-3 border-b hover:bg-blue-50 transition px-2').style(cols):
                    ...
                    with ui.element('div').classes('flex justify-center w-full'): ui.icon('circle', color='green' if n['enable'] else 'red').props('size=xs')
                    lid = f"lat-{n['id']}"; ui.label('...').classes('text-xs text-gray-400 w-full text-center').props(f'id={lid}')
                    if base: ui.run_javascript(f"check_latency('{base}/login', '{lid}')")
                    with ui.row().classes('gap-2 justify-center w-full'):
                        ui.button(icon='edit', on_click=lambda i=n: open_inbound_dialog(mgr, i, lambda: refresh_content('SINGLE', server_conf))).props('flat dense size=sm')
                        ui.button(icon='delete', on_click=lambda i=n: delete_inbound(mgr, i['id'], lambda: refresh_content('SINGLE', server_conf))).props('flat dense size=sm color=red')
    asyncio.create_task(load_nodes())

def render_aggregated_view(server_list):
    list_container = ui.column().classes('w-full gap-4')
    async def load_all():
        list_container.clear(); tasks = [fetch_inbounds_safe(s) for s in server_list]
        ...
        with list_container:
            ...
            with ui.element('div').classes('grid w-full gap-4 font-bold text-gray-500 border-b pb-2 px-2 bg-gray-50').style(cols):
                for h in ['服务器', '名称', '所在组', '协议', '端口', '状态', '延迟', '操作']: ui.label(h).classes('w-full text-center')
            has_data = False
            for i, res in enumerate(results):
                srv = server_list[i]
                if isinstance(res, Exception) or not res: continue 
                has_data = True; mgr = get_manager(srv)
                raw_url = mgr.url if mgr.url else srv['url']
                if '://' not in raw_url: raw_url = f'http://{raw_url}'
                try: base = f"{urlparse(raw_url).scheme}://{urlparse(raw_url).hostname}:{urlparse(raw_url).port or 80}"
                except: base = None
                for n in res:
                    with ui.element('div').classes('grid w-full gap-4 py-3 border-b hover:bg-blue-50 transition px-2').style(cols):
                        ...
                        ui.label(str(n['port'])).classes('text-blue-600 font-mono w-full text-center')
                        with ui.element('div').classes('flex justify-center w-full'): ui.icon('circle', color='green' if n['enable'] else 'red').props('size=xs')
                        lid = f"lat-{srv['url']}-{n['id']}"; ui.label('...').classes('text-xs text-gray-400 w-full text-center').props(f'id={lid}')
                        if base: ui.run_javascript(f"check_latency('{base}/login', '{lid}')")
                        with ui.row().classes('gap-2 justify-center w-full'):
                            ui.button(icon='edit', on_click=lambda m=mgr, i=n, s=srv: open_inbound_dialog(m, i, lambda: refresh_content('SINGLE', s))).props('flat dense size=sm')
                            ui.button(icon='delete', on_click=lambda m=mgr, i=n, s=srv: delete_inbound(m, i['id'], lambda: refresh_content('SINGLE', s))).props('flat dense size=sm color=red')
...
class SubEditor:
    ...
    def ui(self, dlg):
        with ui.card().classes('w-full max-w-4xl h-[70vh] flex flex-col'):
            ui.input('名称', value=self.d['name']).classes('w-full').on_value_change(lambda e: self.d.update({'name':e.value}))
                    ...
                    with cont:
                        with ui.expansion(srv['name']).classes('w-full').props('default-opened'):
                            ...
                            if not res or isinstance(res, Exception): 
                                ui.label('连接失败').classes('text-red-500'); continue
                            for n in res:
                                k=f"{srv['url']}|{n['id']}"
                                def chk(e, key=k): 
                                    if e.value: self.sel.add(key)
            ...
            with ui.card().classes('w-full p-4 mb-2'):
                with ui.row().classes('justify-between w-full items-center'):
                    ...
                    with ui.row():
                        ...
                        async def dl(i=idx):
                            del SUBS_CACHE[i]; await save_subs(); await load_subs_view()
...
async def load_dashboard_stats():
    ...
    for i, res in enumerate(results):
        ...
        try: host = urlparse(srv_url).hostname or srv_url.split('://')[-1].split(':')[0]
        except: host = ""
        ...
        with ui.row().classes('w-full gap-6 mb-6'):
            ...
            with ui.card().classes('w-2/3 p-6 shadow-sm border border-gray-100'):
                ...
                ui.echart({
                    ...
                    'series': [{'type': 'bar', 'data': values, 'itemStyle': {'color': '#6366f1', 'borderRadius': [0, 4, 4, 0]}, 'barWidth': 20}]
                }).classes('w-full h-64')
